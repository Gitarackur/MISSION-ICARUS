...
class PcaHandler(ScientificActionHandler):
    ...
    def run(self, context: ScientificExecutionContext) -> dict:
        ...
        requested = bounded_int(
            context.payload.get("numComponents"), 2, 1, 10_000
        )
        ...
        estimator = PCA(
            ...
            random_state=bounded_int(
                context.payload.get("seed"), 42, 0, 2**32 - 1
            ),
        )
        ...
        if context.action == "pca-analysis" and bool(
            context.payload.get("performClustering", False)
        ):
            clusters = bounded_int(
                context.payload.get("clusters"), 3, 1, scores.shape[0]
            )
            assignments = KMeans(
                n_clusters=clusters,
                n_init=10,
...
class PlsDaHandler(ScientificActionHandler):
    ...
    def run(self, context: ScientificExecutionContext) -> dict:
        ...
        samples = sample_matrix(context.matrix.columns)
        raw_labels = context.payload.get("labels", [])
        ...
        components = min(
            bounded_int(context.payload.get("numComponents"), 2, 1, 10_000),
            samples.shape[1],
...
class TsneHandler(ScientificActionHandler):
    ...
    def run(self, context: ScientificExecutionContext) -> dict:
        ...
        dimensions = bounded_int(
            context.payload.get("numDimensions"), 2, 1, 3
        )
        perplexity = bounded_float(
            context.payload.get("perplexity"),
            30.0,
            1.0,
            max(
                1.0,
        ...
        iterations = bounded_int(
            context.payload.get("iterations"), 1000, 250, 5000
        )
        parameters = {
