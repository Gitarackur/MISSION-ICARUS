...
from analysis.payloads import KnnImputationPayload, MicePayload
...
class MiceImputationHandler(ScientificActionHandler):
    ...
    def run(self, context: ScientificExecutionContext) -> dict:
        payload = cast(MicePayload, context.payload)
        metadata = run_mice(payload, context.emit_progress)
...
class KnnImputationHandler(ScientificActionHandler):
    ...
    def run(self, context: ScientificExecutionContext) -> dict:
        ...
        payload = cast(KnnImputationPayload, context.payload)
        ...
        neighbors = bounded_int(
            payload.get("neighbors"), 5, 1, max(1, samples.shape[0])
        )
