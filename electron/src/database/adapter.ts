...
} from '../../../src/app-layer/database/database.types';
...
export class IcarusDBAdapter {
  ...
  saveSession(session: IcarusSessionRecord): void {
    ...
    const date = typeof session.date === 'string' ? session.date : session.date.toISOString();
  ...
  getSession(id: string): IcarusSessionRecord | null {
    const row = db.prepare(`SELECT * FROM sessions WHERE id = ?`).get(id) as
      | (Omit<IcarusSessionRecord, 'workflowIds' | 'activityIds' | 'matrixIds' | 'visualizationIds' | 'date'> & { 
          workflowIds: string;
          activityIds: string;
          matrixIds: string;
  ...
  getMatrix(id: string): IcarusMatrixRecord | null {
    const row = db.prepare(`SELECT * FROM matrices WHERE id = ?`).get(id) as
      | { 
          id: string; 
          createdAt: number; 
          columns: string; 
  ...
  deleteMatrix(id: string): void {
    db.prepare(`DELETE FROM matrices WHERE id = ?`).run(id);
  }

  // ACTIVITY METHODS
  saveActivity(activity: IcarusActivityRecord): void {
    const inputColumnNames = activity.inputColumnNames ? JSON.stringify(activity.inputColumnNames) : null;
    const outputColumnNames = activity.outputColumnNames ? JSON.stringify(activity.outputColumnNames) : null;
    const inputParameters = activity.inputParameters ? JSON.stringify(activity.inputParameters) : null;
    const outputMetrics = activity.outputMetrics ? JSON.stringify(activity.outputMetrics) : null;
  ...
  getActivity(id: string): IcarusActivityRecord | null {
    ...
    return {
      ...
      inputColumnNames: row.inputColumnNames ? JSON.parse(row.inputColumnNames) : undefined,
      outputColumnNames: row.outputColumnNames ? JSON.parse(row.outputColumnNames) : undefined,
      inputParameters: row.inputParameters ? JSON.parse(row.inputParameters) : undefined,
      outputMetrics: row.outputMetrics ? JSON.parse(row.outputMetrics) : undefined,
  ...
  getVisualization(id: string): IcarusVisualizationRecord | null {
    const row = db.prepare(`SELECT * FROM visualizations WHERE id = ?`).get(id) as
      | { 
          id: string; 
