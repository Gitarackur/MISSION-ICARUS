...
export function rollingStdDev(values: number[], windowSize: number = 5): number[] {
  if (!values.length || windowSize <= 0) return [];
...
export function tTestTwoSample(group1: number[], group2: number[]): TTestResult {
...
export function calculateFoldChange(group1: number[], group2: number[]): FoldChangeResult {
  if (group1.length === 0 || group2.length === 0) {
    throw new Error("Both groups must have at least one value");
  }
  ...
  if (mean2 === 0) {
    throw new Error("Cannot calculate fold change when control group mean is zero");
  }
...
export function limmaAnalysis(
  ...
  const standardError = Math.sqrt(pooledVar * (1/treatmentGroup.length + 1/controlGroup.length));
...
export function normalizeReporterIons(
  ...
  method: 'median' | 'mean' | 'total' = 'median'
  ...
  for (let i = 0; i < numChannels; i++) {
    const channelData = reporterIonData[i].filter(val => !isNaN(val) && val > 0);
...
export function correctForPurity(
  ...
  if (matrix.length !== numChannels) {
    throw new Error(`Purity matrix dimensions (${matrix.length}x${matrix[0]?.length}) do not match number of channels (${numChannels})`);
  }
  ...
  for (let pointIdx = 0; pointIdx < numDataPoints; pointIdx++) {
    const observedIntensities: number[] = reporterIonData.map(channel => channel[pointIdx]);
    
    // Simple correction: divide by main diagonal (approximation)
    // For more accurate correction, implement proper matrix inversion
    const correctedIntensities = observedIntensities.map((intensity, channelIdx) => {
...
export function calculateChannelRatios(
  ...
  if (referenceChannelIndex < 0 || referenceChannelIndex >= reporterIonData.length) {
    throw new Error(`Invalid reference channel index: ${referenceChannelIndex}`);
  }
...
export function reorderColumns(
  ...
  if (newOrder.length !== data.length) {
    throw new Error(`New order length (${newOrder.length}) must match number of columns (${data.length})`);
  }
...
export function filterColumnsByName(
  ...
  data: number[][],
  pattern: string,
  matchType: 'contains' | 'starts' | 'ends' | 'exact' = 'contains',
...
export function filterColumnsByType(
  columnNames: string[],
  data: number[][],
  targetType: 'numeric' | 'integer' | 'float' | 'positive' | 'negative' | 'nonzero'
): FilterByTypeResult {
  if (!columnNames || columnNames.length === 0) {
    throw new Error("No column names provided");
  }
...
export function performPCA(
  ...
  for (let i = 0; i < numComponents; i++) {
    ...
    for (let k = 0; k < i; k++) {
      const dotProduct = component.reduce((sum, val, idx) => 
        sum + val * components[k][idx], 0);
      component = component.map((val, idx) => 
        val - dotProduct * components[k][idx]);
    }
    ...
    if (norm > 0) {
      ...
      component = component.map(() => (Math.random() - 0.5) / Math.sqrt(numFeatures));
    }
  ...
  const explained_variance = totalVariance > 0 
...
export function performPLSDA(
  ...
  if (labels.length !== numSamples) {
    throw new Error(`Number of labels (${labels.length}) must match number of samples (${numSamples})`);
  }
  ...
  for (let i = 0; i < numComponents; i++) {
    ...
    for (let j = 0; j < numFeatures; j++) {
      ...
      uniqueLabels.forEach(label => {
        const classIndices = labels.map((l, idx) => l === label ? idx : -1).filter(idx => idx >= 0);
        const classValues = classIndices.map(idx => centeredData[j][idx]).filter(v => isFinite(v));
      ...
      component[j] = withinClassVar > 0 ? betweenClassVar / withinClassVar : betweenClassVar;
    }
  ...
  if (transformed_data.length > 0) {
    ...
    uniqueLabels.forEach((label1, idx1) => {
      uniqueLabels.forEach((label2, idx2) => {
        if (idx1 < idx2) {
          const class1Indices = labels.map((l, i) => l === label1 ? i : -1).filter(i => i >= 0);
          const class2Indices = labels.map((l, i) => l === label2 ? i : -1).filter(i => i >= 0);
    ...
    separationScore = Math.min(separationScore / ((numClasses * (numClasses - 1)) / 2), 1);
  }
...
export function performTSNE(
  ...
  // Validate perplexity
  const safePerplexity = Math.min(Math.max(perplexity, 5), Math.floor(numSamples / 3));
  ...
  const momentum = 0.5;
  let previousGradient: number[][] = embedded_data.map(dim => dim.map(() => 0));
  ...
  for (let iter = 0; iter < Math.min(iterations, 1000); iter++) {
    ...
    for (let i = 0; i < numSamples; i++) {
      for (let j = 0; j < numSamples; j++) {
        ...
        const highDimSimilarity = Math.exp(-distances[i][j] * distances[i][j] / (2 * safePerplexity));
        const lowDimSimilarity = 1 / (1 + lowDimDist * lowDimDist);
        
        const force = (highDimSimilarity - lowDimSimilarity) / (lowDimDist + 1e-10);
    ...
    for (let d = 0; d < numDimensions; d++) {
      for (let i = 0; i < numSamples; i++) {
        ...
        const update = currentMomentum * previousGradient[d][i] - currentLearningRate * gradient;
  ...
  for (let d = 0; d < numDimensions; d++) {
    ...
    if (validValues.length > 0) {
      ...
      embedded_data[d] = embedded_data[d].map(val => (val - dimMean) / safeStd);
    }
  }
